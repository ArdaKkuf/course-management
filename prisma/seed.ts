...
async function main() {
  ...
  await prisma.etutRandevu.deleteMany()
  await prisma.odev.deleteMany()
  await prisma.musaatlik.deleteMany()
  await prisma.dersProgrami.deleteMany()
  await prisma.ogrenci.deleteMany()
  await prisma.ogretmen.deleteMany()
  ...
  // Create Teachers
  const ogretmenler = [
    { email: 'ahmet@dershane.com', name: 'Ahmet Yılmaz', brans: 'Matematik', password: hocaPassword },
  ...
  for (const odev of sampleOdevler) {
    ...
    await prisma.odev.create({
      data: {
        ...
        sinif: odev.sinif,
        sonTarih: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 days from now
